...
export default async function handler(req, res) {
  try {
    ...
    const url = `https://graph.facebook.com/v19.0/${adAccountId}/adsets?fields=id,name,status,objective,campaign_id,insights.time_range({"since":"${since}","until":"${until}"}){impressions,reach,clicks,ctr,cpc,cpm,spend,actions}&limit=50`;
    ...
    const adsets = json.data.map((adset) => {
      const insights = adset.insights?.data?.[0] || null;
      return {
        ...
        status: adset.status,
        resultado: getCorrectMetric(adset.objective, insights),
